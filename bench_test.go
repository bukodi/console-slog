...
var handlers = []struct {
	...
	{"console-headers", NewHandler(io.Discard, &HandlerOptions{HeaderFormat: "%t %{%[foo]h > %}%l %m", Level: slog.LevelDebug, AddSource: false})},
	...
	{"console-headers-replaceattr", NewHandler(io.Discard, &HandlerOptions{HeaderFormat: "%t %{%[foo]h > %} %l %m", Level: slog.LevelDebug, AddSource: false, ReplaceAttr: func(_ []string, a slog.Attr) slog.Attr { return a }})},
